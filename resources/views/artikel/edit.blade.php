...
@section('content')
<h1 class="text-2xl font-bold mb-6">Edit Artikel: {{ $artikel->title }}</h1>

<form id="content-form" action="{{ route('pk.artikel.update', $artikel) }}" method="POST" enctype="multipart/form-data"
    ...
    <div>
        ...
        <input type="text" name="title" value="{{ old('title', $artikel->title) }}" class="w-full border rounded px-3 py-2">
    </div>
    ...
    <div>
        ...
        <select name="category_id" class="w-full border rounded px-3 py-2">
            ...
            @foreach ($categories as $category)
                <option value="{{ $category->id }}" @selected(old('category_id', $artikel->category_id) == $category->id)>
                    {{ $category->name }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <textarea name="excerpt" rows="2" class="w-full border rounded px-3 py-2">{{ old('excerpt', $artikel->excerpt) }}</textarea>
    </div>

    <div>
        <label class="block text-sm font-medium mb-1">Isi Artikel</label>
        <div id="editor" style="height: 250px;" class="bg-white"></div>
        <textarea name="content" id="content" class="hidden">{{ old('content', $artikel->content) }}</textarea>
    </div>
    ...
    <div>
        ...
        <select name="penulis[]" multiple class="w-full border rounded px-3 py-2 h-32">
            @foreach ($users as $user)
                <option value="{{ $user->id }}" @selected($artikel->penulis->contains($user->id))>
    ...
    <div>
        ...
        <div class="flex flex-wrap gap-3">
            @forelse ($artikel->images as $image)
                <div class="relative">
                    <img src="{{ asset('storage/' . $image->file_path) }}" class="w-24 h-24 object-cover rounded border">
                    <form action="{{ route('pk.artikel.images.destroy', [$artikel, $image]) }}" method="POST"
    ...
    <div>
        ...
        <select name="status" class="w-full border rounded px-3 py-2">
            ...
            <option value="review" @selected(old('status', $artikel->status) === 'review')>Review</option>
    ...
    <a href="{{ route('pk.artikel.index') }}" class="text-sm underline ml-2">Batal</a>
</form>
