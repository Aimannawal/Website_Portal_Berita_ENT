...
<div class="px-5 py-8 lg:px-8">
    <div class="mb-6"><p class="text-xs font-bold uppercase tracking-[.18em] text-[#6d94d3]">People & access</p><h1 class="mt-1 text-2xl font-bold">Tambah user</h1><p class="mt-1 text-sm text-[#64748b]">Pilih divisi. Role akan diatur otomatis oleh sistem.</p></div>
    <form id="user-form" action="{{ route('wm.users.store') }}" method="POST" class="portal-card max-w-2xl space-y-5 p-6">
        ...
        <div><label for="division_id" class="mb-1 block text-sm font-semibold">Divisi</label><select id="division_id" name="division_id" class="portal-input" required><option value="">Pilih divisi</option>@foreach ($divisions as $division)<option value="{{ $division->id }}" @selected(old('division_id') == $division->id)>{{ $division->name }}</option>@endforeach</select><p class="mt-1 text-xs text-[#64748b]">Role otomatis mengikuti divisi yang dipilih.</p></div>
